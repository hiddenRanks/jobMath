...
router.post("/datalab2", function (req, res) {
    let data = [
        { "groupName": req.body.searchOne, "keywords": [] },
        { "groupName": req.body.searchTwo, "keywords": [] }
    ];

    let oneWord = req.body.oneKey.split(",");
    let twoWord = req.body.twoKey.split(",");

    if (oneWord.length > 10 || twoWord.length > 10) {
        //키워드가 10개 이상일시 10개만 받고 끝냄
        for (let i = 0; i < oneWord.length; i++) {
            if(i <= 9) {
                data[0].keywords[i] = oneWord[i];
            }
        }

        for (let i = 0; i < twoWord.length; i++) {
            if(i <= 9) {
                data[1].keywords[i] = twoWord[i];
            }
        }
    }
    else {
